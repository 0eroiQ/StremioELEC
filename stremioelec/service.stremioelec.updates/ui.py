...
def _download_candidates(updater, dialog):
    ...
    for kind, entry in candidates.items():
        ...
        if not dialog.yesno(title,
                'Download StremioELEC ' + entry['version'] +
    ...
    if downloaded:
        dialog.notification('StremioELEC', 'Verified update ready. Install when you are ready to restart.', time=7000)
...
def main(action='open'):
    ...
    try:
        ...
        if action == 'refresh':
            publish(updater)
            return
        if action in ('toggle_os', 'toggle_addons'):
            with updater.locked():
